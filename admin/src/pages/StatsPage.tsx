...
import { BarChart3, Activity, Send, MessageSquare, Cpu } from 'lucide-react'
...
export default function StatsPage() {
  ...
  const providers = useQuery({ queryKey: ['stats', 'providers'], queryFn: () => api.get<ProviderStat[]>('/api/stats/providers') })
      ...
      {providers.data && providers.data.length > 0 ? (
        <div className="card" style={{ overflow: 'hidden', padding: 0, maxWidth: 640 }}>
          {providers.data.map((p, i) => (
            <div key={p.name} style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '0.75rem 1rem', borderBottom: i < providers.data.length - 1 ? '1px solid var(--glass-border)' : 'none' }}>
