...
func (d *DB) Migrate() error {
	...
	mtypeRows, err := d.Query(`PRAGMA table_info(media_group_items)`)
	if err != nil {
	...
	for mtypeRows.Next() {
		...
		var dfltValue interface{}
		var pk int
		if err := mtypeRows.Scan(&cid, &name, &ctype, &notnull, &dfltValue, &pk); err != nil {
			mtypeRows.Close()
