...
var AvailableTools = []llm.Tool{
	...
	},
	{
		...
		Description: "Переслать (скопировать) сообщение в другой топик (тему, ветку, раздел). Пользователи могут сказать: 'перешли это в топик X', 'скинь в тему Y', 'отправь в ветку Z', 'перешли в сессию', 'в топик расписание'. ВАЖНО: если указанный топик не найден — не создавай его, а сообщи пользователю.",
		Parameters: map[string]interface{}{
			"type": "object",
			"properties": map[string]interface{}{
